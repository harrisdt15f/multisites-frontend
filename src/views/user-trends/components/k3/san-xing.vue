    ...
    >
      <thead class="thead">
        <tr class="title-text">
          ...
          <th colspan="8" class="border-right">百位</th>
          <th colspan="8" class="border-right">十位</th>
          <th colspan="8" class="border-right">个位</th>
          <th colspan="8" class="border-right">号码分布</th>
          <th rowspan="2" class="border-bottom border-right">和值</th>
          <th rowspan="2" class="border-bottom border-right">豹子(三同号)</th>
          <th rowspan="2" class="border-bottom border-right">对子(二同号)</th>
          <th rowspan="2" class="border-bottom border-right">三不同</th>
          <th rowspan="2" class="border-bottom border-right">三连号</th>
        </tr>
        <tr class="title-number">
          ...
          <template v-for="item in 4">
            ...
            <th class="border-bottom-header td-bg" v-for="num in 6" :key="`${item}-${num}`">
      ...
      <tbody ref="chart-content" class="chart" :class="{'table-guides':showGuides}">
        ...
        >
          ...
          <td class="ball-none border-right"></td>
          <!-- 和值 -->
          <td class="border-right">{{item[6].join('')}}</td>
          <!-- 豹子/三同号 -->
          <td class="border-right">
            <template v-if="item[7][0] == 0">
              <span class="group-current"></span>
            </template>
            <template v-else>{{item[7][0]}}</template>
          </td>
          <!-- 对子/二同号 -->
          <td class="border-right">
            <template v-if="item[8][0] == 0">
              <span class="group-current"></span>
            </template>
            <template v-else>{{item[8][0]}}</template>
          </td>
          <!-- 三不同 -->
          <td class="border-right">
            <template v-if="item[9][0] == 0">
              <span class="group-current"></span>
            </template>
            <template v-else>{{item[9][0]}}</template>
          </td>
          <!-- 三连号 -->
          <td class="border-right">
            <template v-if="item[10][0] == 0">
              <span class="group-current"></span>
            </template>
      ...
      <tbody class="tbody ball-content">
        <tr class="auxiliary-area">
          ...
          <template v-if="totalNum[4]">
            <td
              class="border-bottom border-right"
              v-for="(item, index) in totalNum[4]"
              :key="`${index}-t1`"
            >
              <i class="ball-noraml">{{item}}</i>
            </td>
        ...
        <tr class="auxiliary-area">
          ...
          <template v-if="omissionNum[4]">
            <td
              class="border-bottom border-right"
              v-for="(item, index) in omissionNum[4]"
              :key="`${index}-t2`"
            >
              <i class="ball-noraml">{{item}}</i>
            </td>
        ...
        <tr class="auxiliary-area">
          ...
          <template v-if="omissionMaxNum[4]">
            <td
              class="border-bottom border-right"
              v-for="(item, index) in omissionMaxNum[4]"
              :key="`${index}-t3`"
            >
              <i class="ball-noraml">{{item}}</i>
            </td>
        ...
        <tr class="auxiliary-area">
          ...
          <template v-if="continuousNum[4]">
            <td
              class="border-bottom border-right"
              v-for="(item, index) in continuousNum[4]"
              :key="`${index}-t4`"
            >
              <i class="ball-noraml">{{item}}</i>
            </td>
      ...
      <tbody class="tbody tbody-footer-header">
        <tr class="auxiliary-area title-number">
          ...
          <template v-for="items in 4">
            ...
            <td v-for="item in 6" :key="`${items}-${item}`" class="border-bottom td-bg">
              <i class="ball-noraml">{{item}}</i>
            </td>
        ...
        <tr class="auxiliary-area title-text">
          <td colspan="8" class="border-right border-bottom">百位</td>
          <td colspan="8" class="border-right border-bottom">十位</td>
          <td colspan="8" class="border-right border-bottom">个位</td>
          <td colspan="8" class="border-right border-bottom">号码分布</td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
  ...
  methods: {
    handleDrawing(list) {
      ...
      this.totalNum = this._.chunk(sumData[0], 6);
      this.omissionNum = this._.chunk(sumData[1], 6);
      this.omissionMaxNum = this._.chunk(sumData[2], 6);
      this.continuousNum = this._.chunk(sumData[3], 6);
